...
func (h *Handler) Create(ctx *gin.Context) {
	...
	param.UserID = ctx.Value("user_id").(int64)
...
func (h *Handler) Void(ctx *gin.Context) {
	...
	if err := h.Usecase.Void(id); err != nil {
		ctx.JSON(http.StatusInternalServerError, presenter.Default{
			Message: err.Error(),
		})
		return
	...
	ctx.JSON(http.StatusOK, presenter.Default{
		Message: fmt.Sprintf("%s %s", presenter.RESPONSE_SUCCESS_VOID, h.Name),
	})
}
